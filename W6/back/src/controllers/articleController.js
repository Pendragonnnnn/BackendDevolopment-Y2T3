...
export async function getArticleWithJournalistId(req, res) {
  try {
    const article = await articleRepository.fetchArticlesWithId(req.params.id);
    if (!article) {
      return res.status(404).json({ message: "Article not found" });
    }
    res.json(article);
...
export async function getAllArticleByJournalistId(req, res) {
  try {
    const articles = await articleRepository.fetchAllArticleByJournalist(req.params.id);
